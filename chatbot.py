...
def synthesize_speech(text, language_code):
    if text and language_code in ['en', 'te']:
        try:
            cleaned_text = text.replace('*', '')  # Ensure * is removed from the speech text
...
def chat():
    try:
        ...
        if audio_data:
            # Placeholder for audio-to-text conversion (requires a separate service)
        ...
        # Remove * from response text before formatting
        chatbot_response_en = chatbot_response_en.replace('*', '')  # Remove * from raw response text


        # Replace * with <strong> tags for formatting in HTML
        ...
        # Remove * from translated response
        translated_response = translated_response.replace('*', '')  # Remove * from the translated text
